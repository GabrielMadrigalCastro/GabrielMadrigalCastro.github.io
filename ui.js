...
function nextYear(){
    ...
    // Initialize immigrants for first year
    if(year === 1) {
        immigrants = 5
    }
...
input.addEventListener("keydown",function(e){
    ...
    if(step==="buy"){
        ...
        eatenByRats = 0
        if(rand(2) === 0){
            eatenByRats = Math.floor(grain / (rand(5)+2)) // Avoid division by zero
        }
        
        grain = grain - eatenByRats + harvest
        
        // Fix immigrant calculation to avoid division by zero
        if(population > 0) {
